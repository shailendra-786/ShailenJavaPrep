...
public class Test {
	...
	public static int strongPasswordChecker(String password) {
		...
		int one = 0, two = 0; // number of replacements needed for sequences of 3 or more
		int[] seqs = new int[n]; // lengths of repeated sequences
		...
		if (n < 6) {
			...
			return Math.max(missing, one + two - (n - 20) / 3);
		} else {
			int delete = n - 20;
			PriorityQueue<Integer> pq = new PriorityQueue<>();
			for (int len : seqs) {
				if (len >= 3 && len - 2 <= delete) {
					pq.offer(len - 2);
					delete -= len - 2;
				}
			}
			int change = 0;
			while (delete > 0 && !pq.isEmpty()) {
				int len = pq.poll();
				int count = Math.min(len - 1, delete);
				delete -= count;
				len -= count;
				if (len >= 3)
					pq.offer(len);
			...
			return delete + Math.max(missing, one + two) + change;
		}
	}
